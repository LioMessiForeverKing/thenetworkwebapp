...
export default function OnboardingPage() {
  ...
  const [hasAgreed, setHasAgreed] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('consent_agreed') === 'true';
    }
    return false;
  });
    ...
    <div className="min-h-screen bg-white flex flex-col items-center pt-8 md:pt-12 pb-24 md:pb-4 px-4 gap-4 overflow-x-hidden relative">
      ...
      <div className="flex flex-col items-center gap-4 pb-8 md:pb-12 fixed bottom-0 left-0 right-0 pointer-events-none bg-gradient-to-t from-white via-white to-transparent pt-8">
        <div className="pointer-events-auto flex items-center gap-2 mb-2">
          ...
          />
          <label htmlFor="consent-checkbox" className="text-sm font-medium text-gray-600 cursor-pointer select-none">
            I agree to the <a href="/terms-of-service" target="_blank" className="underline hover:text-black">Terms of Service</a> and <a href="/privacy-policy" target="_blank" className="underline hover:text-black">Privacy Policy</a>
          </label>
        </div>

        <button
          onClick={handleContinue}
          className={`pointer-events-auto relative group transition-all duration-200 ${!hasAgreed ? 'opacity-50 grayscale' : 'opacity-100'} ${shake ? 'animate-shake' : ''}`}
        >
